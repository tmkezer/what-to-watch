...
def most_similar_users_(user1, number_of_users, index, metric='pearson'):
    ...
    sid=index
    eid=index+10
    print("sid and eid: ", sid, eid)
    print("user[sid:eid] ", user_ids[sid:eid])
...
ratings = pd.read_csv("./data/ml-latest/ratings.csv", low_memory=False, encoding = 'latin-1')
...
users_reccs.to_csv("./output/usrecc_1.csv")
